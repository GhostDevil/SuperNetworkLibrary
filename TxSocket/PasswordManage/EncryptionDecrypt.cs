...
namespace SuperNetwork.TxSocket.PasswordManage
{
    ...
    internal class EncryptionDecrypt
    {
        ...
        internal static byte[] encryption(TxStateCode stateCode, StateBase state)
        {
        ...
        private static byte[] encryptionTemporary(byte[] date, byte textCode, StateBase state)
        {
            ...
            byte[] dateOverall = ByteToData.OffsetEncryption(date, state.SendDateLabel, 2);
        ...
        internal static TxStateCode deciphering(byte[] date, StateBase state)
        {
            ...
            if (headDate == PasswordCode._textCode || headDate == PasswordCode._photographCode)//解密到的是文本数据
            {
                ...
                byte[] ReplyDate = ByteToData.CombinationTwo(PasswordCode._commonCode, PasswordCode._dateSuccess, SendDateLabel);//直接回复发送成功
                if (headDate == PasswordCode._textCode)
                {
                    string str = Encoding.UTF8.GetString(dateAll);
                    stateCode = new TxStateCode(PasswordCode._textCode, str, ReplyDate);//解析出来是文本数据
                }
                else
                {
                    stateCode = new TxStateCode(PasswordCode._photographCode, dateAll, ReplyDate);//解释出来是图片数据
                }

            }
            else if (headDate == PasswordCode._dateSuccess)//数据成功或重发
            {
                int SendDateLabel = ByteToData.ByteToInt(2, date);//找出已发数据的标签
                if (headDate == PasswordCode._dateSuccess)
                {
                    stateCode = new TxStateCode(headDate);
