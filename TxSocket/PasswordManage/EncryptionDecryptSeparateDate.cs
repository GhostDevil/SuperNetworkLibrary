...
namespace SuperNetwork.TxSocket.PasswordManage
{
    ...
    internal class EncryptionDecryptSeparateDate
    {
       ...
       {
           byte[] dateOverall = ByteToData.OffsetEncryption(date, sendDateLabel,2);
       ...
       internal static TxStateCode FileDecrypt(byte[] date, StateBase state)
       {
           ...
           {//对方同意接收文件;我应该怎么处理
               int FileLabel = ByteToData.ByteToInt(2, date);
               ...
               if (state.SendFile != null && state.SendFile.FileLabel == FileLabel)
               {
                   ...
                   if (SendSubjectDate == null)
                       stateCode = new TxStateCode(PasswordCode._dateSuccess);
                   else
                       stateCode = new TxStateCode(SendSubjectDate);//直接发送
               }
           }
           ...
           {//收到的是文件包头部分
               ...
               int fileLabel = ByteToData.ByteToInt(3, date);
               int fileLenth = ByteToData.ByteToInt(7, date);
               ...
               ByteToData.IntToByte(fileLabel, 2, dateAll);
               stateCode = new TxStateCode(dateAll);
           }
           else if (headDate == PasswordCode._fileSubjectCode)
           {//收到的是文件主体部分
               int SendDateLabel = 0;
               byte[] dateAll = ByteToData.OffsetDecrypt(date, out SendDateLabel, 2);
               byte[] ReplyDate = ByteToData.CombinationTwo(PasswordCode._bigDateCode, PasswordCode._dateSuccess, state.ReceiveFile.FileLabel);
               if (state.ReceiveFile.FileDateAll == null)
               {
                   state.ReceiveFile.FileDateAll = dateAll;//是第一次接收到主体数据
                   stateCode = new TxStateCode(ReplyDate); 
               }
               else
               {
                   ...
                   if (FileDateAll.Length == state.ReceiveFile.FileLenth)
                   {
                       if (state.ReceiveFile.FileClassification == PasswordCode._textCode)
                       {
                           ...
                           stateCode = new TxStateCode(PasswordCode._textCode, str, ReplyDate);
                       }
                       else
                       {
                           stateCode = new TxStateCode(PasswordCode._photographCode, FileDateAll, ReplyDate);
                       }
