...
namespace SuperNetwork.TxSocket.PasswordManage
{
    ...
    internal class StickPackage
    {
        ...
        internal static void EncryptionPackage(ref byte[] sendDate)
        {
            ...
            ByteToData.IntToByte(PasswordCode._stickPackageCode, 0, dateAll);
            ByteToData.IntToByte(sendDate.Length, 4, dateAll);
        ...
        private static byte[] DecryptByte(byte[] receiveDate, ref byte[] residualpackage)
        {
            byte[] haveDate = null;
            bool ddd=false;
            int stickPackageCode = ByteToData.ByteToInt(0, receiveDate);
            ...
            if (ddd)
                stickPackageCode = ByteToData.ByteToInt(0, receiveDate);
            if (stickPackageCode == PasswordCode._stickPackageCode)
            { //第一个暗号相同，说明是一个新包开始了；不会是一个前面的残留包
                if (receiveDate.Length < 9)
                { residualpackage = receiveDate; return null; }
                int datelenth = ByteToData.ByteToInt(4, receiveDate);
